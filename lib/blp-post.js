...
import { requireAuth, cors, jsonErr } from './auth.js';
import { getProfile, formatContent } from './site-profiles.js';
...
async function injectCookies(ctx, url, cookieStr) {
  // Supports "name=value; name2=value2" format or "name=value; Path=/; Domain=..."
  const domain = new URL(url).hostname;
  // Split on semicolons but only take name=value pairs (skip Path, Domain, etc.)
  ...
  cookieStr.split(';').forEach(pair => {
    ...
    // Skip cookie attribute keys
    if (/^(path|domain|expires|max-age|samesite|httponly|secure)$/i.test(name)) return;
